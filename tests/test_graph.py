...
def test_node(mmodel_graph):
    mmodel_graph.copy()


def test_default_mockgraph(mmodel_graph, standard_graph):
    """Test if default ModelGraph matches the ones created by DiGraph"""

    # assert graph equal
    graphs_equal(mmodel_graph, standard_graph)


def test_add_nodes(mmodel_graph, standard_graph):
    """Test add_node and add_nodes_from methods

    The result three graph graph_a, graph_b and standard_graph
    should be the same.
    """
    ...
    graph_a = mmodel_graph.copy()
    graph_b = mmodel_graph.copy()
    ...
    graphs_equal(graph_a, standard_graph)


def test_add_nodes_fails(mmodel_graph):
    ...
    with pytest.raises(Exception, match="Node list missing node attributes"):
        mmodel_graph.add_nodes_from([("mock_func")])
    with pytest.raises(Exception, match="Node list missing attribute node_obj or return_params"):
        mmodel_graph.add_nodes_from([("mock_func", {})])


def test_add_nodes_object_type_fails(mmodel_graph):

    # node object should be a callable
    with pytest.raises(Exception, match="Node object type <class 'str'> not supported"):
        mmodel_graph.add_node("mock_func", "obj", [])
    with pytest.raises(Exception, match="Node object type <class 'str'> not supported"):
        mmodel_graph.add_nodes_from([("mock_func", {"node_obj": "obj", "return_params": []})])


def test_add_edges(mmodel_graph, standard_graph):
    ...
    standard_graph.add_edge("add", "poly", interm_params=["c"])
...
def test_add_edge_fails_nodes(mmodel_graph, edge):
...
def test_add_edges_from_fails_nodes(mmodel_graph, edge_list):
    """Test if `add_edge`, `add_edges_from` fails when edge nodes are not defined"""

    with pytest.raises(Exception, match="Node mock_func is not defined"):
        mmodel_graph.add_edges_from(edge_list)

def test_add_edges_fails_nodes(mmodel_graph):
    """Test if `add_edge`, `add_edges_from` fails when missing edge attributes"""

    with pytest.raises(Exception):
        mmodel_graph.add_edges_from([("add", "poly")])
...
def test_str_named(mmodel_graph):
    """Test the string value"""
    assert (
        str(mmodel_graph)
        == f"{type(mmodel_graph).__name__} named 'test' with 5 nodes and 5 edges"
    )


def test_graph_chain(mmodel_graph):
    """Test Chain graph"""
...
def test_subgraph(mmodel_graph):
    """Test subgraph view"""
    G = mmodel_graph
...
def test_subgraph_copy(mmodel_graph):
